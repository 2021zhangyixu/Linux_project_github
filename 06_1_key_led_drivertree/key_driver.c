...
static ssize_t gpio_drv_read (struct file *file, char __user *buf, size_t size, loff_t *offset)
{
	...
	int ret,i;   //没有使用，用于存放copy_from_user和copy_to_user的返回值，消除报错
	...
	for(i = 0;i < count;i++)
	{
		tmp_buf[i] = gpio_get_value(gpios[i].gpio);
...
static int key_drver_probe(struct platform_device *pdev)
{
	...
	*/
	...
	printk("key_GPIO_set is ok\r\n");

	/* 注册file_operations 	*/
	//注册字符驱动程序
	major = register_chrdev(0, "100ask_key", &gpio_key_drv);  /* /dev/gpio_desc */
	 ...
	 *参数四 ： 没有私有数据
	*/
	device_create(gpio_class, NULL, MKDEV(major, 0), NULL, "100ask_key"); /* /dev/100ask_key */
...
static int key_drver_remove(struct platform_device *pdev)
{
	...
	*/
	...
	printk("The key driver is uninstalled\n");
	return 0;
}


static const struct of_device_id key_dtb[] = 
{
	{.compatible = "key_devicetree", }, //要求和设备树的compatible名字一样
	{ /* sentinel */ }
};
...
static struct platform_driver key_driver = {
	.driver		= {
		.owner = THIS_MODULE,
		.name	= "key_device",   //根据这个名字，找到设备
		.of_match_table = key_dtb, //与设备树有关
	},
	.probe		= key_drver_probe,   //注册平台之后，内核如果发现支持某一个平台设备，这个函数就会被调用。入口函数
	.remove		= key_drver_remove,  //出口函数
};


//注册时候的入口函数
static int __init key_drver_init(void)
{
	int ret = 0;
	// platform驱动注册到 Linux 内核
	ret = platform_driver_register(&key_driver);  //注意，这里是driver表示是驱动
	if(ret<0)
	{
		printk("key_driver_register error \n");
		return ret;
	}
	printk("key_driver_register ok \n");
	return ret;
}

//卸载时候的出口函数
static void __exit key_drver_exit(void)
{
	/* 反注册platform_driver */
	platform_driver_unregister(&key_driver);   //注意，这里是driver表示是驱动
	printk("key_driver_unregister ok \n");
}


module_init(key_drver_init);  //确认入口函数
module_exit(key_drver_exit);  //确认出口函数
